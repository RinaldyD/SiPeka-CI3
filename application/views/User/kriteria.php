                    ...
                    <table class="table table-hover table-bordered ">
                        ...
                        <tr class="font-weight-bold text-grey text-center">
                                ...
                                <td class="py-3"><?php echo $k->nama_kriteria ?></td>
                                    <td class="py-3"><?php echo $k->jenis_kriteria ?></td>
                                    <?php  if ($role == "HRD"){ ?>
                                    ...
                                    <td><a href="<?php echo base_url('user/hapuskriteria/'. $k->id);?>"><button type="button" class="btn btn-danger py-1">DELETE</button></a></td>
                                    <?php } ?>
                            </tr>
                            <?php } ?>
                        </tbody>
...
<div class="modal fade" id="kriteriaModal" tabindex="-1" role="dialog" aria-labelledby="kriteriaModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="<?= base_url('user/kriteria')?>" method="POST">
      <div class="modal-body">
        <div class="form-group">
            <label for="kriteria">Nama Kriteria</label>
            <input class="form-control mb-2" type="text" placeholder="Nama Kriteria" id="kriteria" value="<?= set_value('kriteria');?>" name="kriteria">
            <?=form_error('kriteria', '<small class="text-danger px-2">', '</small>' );?>
        </div>
        <div class="form-group">
            <label for="jeniskriteria">Jenis Kriteria</label>
            <select class="form-control" id="jeniskriteria" name="jeniskriteria" value="<?= set_value('jeniskriteria');?>">
...
<div class="modal fade" id="editkriteriaModal<?= $k->id;?>" tabindex="-1" role="dialog" aria-labelledby="editkriteriaModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="<?= base_url('user/ubahkriteria/'. $k->id)?>" method="POST">
      <div class="modal-body">
        <div class="form-group">
            <input class="form-control mb-2" type="hidden" placeholder="ID Kriteria" id="idkriteria" value="<?= $k->id;?>">
            <label for="kriteria">Nama Kriteria</label>
            <input class="form-control mb-2" type="text" name="nama_kriteria" placeholder="Nama Kriteria" id="kriteria" value="<?= $k->nama_kriteria;?>" disabled>
        </div>
        <div class="form-group">
            <label for="jeniskriteria">Jenis Kriteria</label>
            <select class="form-control" id="jeniskriteria" id="jeniskriteria" name="jeniskriteria">
                <option <?php if ($k->jenis_kriteria == 'Cost') {echo 'selected';}?> value="Cost">Cost</option>
                <option <?php if ($k->jenis_kriteria == 'Benefit') {echo 'selected';}?> value="Benefit">Benefit</option>
            </select>
        </div>
      </div>
      <div class="modal-footer">
